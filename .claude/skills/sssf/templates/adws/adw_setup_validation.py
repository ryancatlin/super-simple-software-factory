...
3. Use the factory's verbs instead of hand-rolling curl: `source "$FLOW_LIB/http.sh"` gives `fetch_expect <url> <outfile> <status> [max_seconds] [required_string ...]`; `source "$FLOW_LIB/browser.sh"` gives `have_browser`, `require_browser`, `capture <name> <url>`, `capture_wait <name> <url> <pattern> [timeout]`. They already assert status, non-empty body, content and deadline, and their degrade path still asserts — a hand-rolled `else echo "no browser"` branch skips the journey's only check and still exits 0. Never assert a frozen count or frozen prose, never assert against anything under adws/adw_data/sessions/ (frozen run artifacts — the lint rejects it), and never point an unreachable-target probe at a real hostname (use an RFC 5737 address like 192.0.2.1; a negative probe also needs a positive control proving the instrument sees the app working).
4. Do NOT start the server or run the flows yourself — this run provisions, captures, and tears down in code immediately after you report.
