...
2. Write one flow per journey worth evidence in adws/adw_data/validation/flows/, replacing or extending the stamped home.sh. Every flow: declared in validation.yaml (undeclared scripts never run and fail the library lint); opening lines carry `# Flow: <name> — <scenario it evidences>`; one journey per flow; MECHANICAL capture only (curl, agent-browser) run with $BASE_URL and $EVIDENCE_DIR set and cwd = $EVIDENCE_DIR; save evidence there; exit non-zero on any checkable failure. Judgement belongs to the validator, not the flow.
