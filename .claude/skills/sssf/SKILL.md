...
11. **The commit phase stages the whole working tree** (`git add -A` — see `adw_modules/git_helper.py`). Anything on disk that is not gitignored gets committed, and `git rm --cached` does NOT survive a commit phase. To keep a file out of commits: gitignore it or delete it. Do not scope the add to the builder's `changed_files` — that list is self-reported and unverified against the diff, so scoping would silently drop unclaimed changes from commits.
